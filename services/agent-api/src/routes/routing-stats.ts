...
// GET /api/routing-stats/:agentId - Get routing statistics for an agent
router.get('/:agentId', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Agent API] GET /routing-stats/:agentId error:', error);
    res.status(500).json({ error: error.message || 'Failed to get routing stats' });
  }
});

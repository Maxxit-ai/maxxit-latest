...
try:
    ...
    print(
        "WARNING: psutil not installed. Resource monitoring disabled. Run: pip install psutil"
    )
...
try:
    ...
    print(
        "WARNING: python-dotenv not installed. To use .env file, run: pip install python-dotenv"
    )
...
try:
    ...
    print(
        "ERROR: ostium-python-sdk-test not installed. Run: pip install ostium-python-sdk-test"
    )
    exit(1)
...
if OSTIUM_MAINNET:
    ...
    OSTIUM_RPC_URL = os.getenv(
        "OSTIUM_RPC_URL", "https://sepolia-rollup.arbitrum.io/rpc"
    )
    OSTIUM_RPC_BACKUP = os.getenv(
        "OSTIUM_RPC_BACKUP", "https://sepolia-rollup.arbitrum.io/rpc"
    )
...
def get_network_config(request_obj=None, is_testnet_override=None):
    ...
    if network == "testnet":
        rpc_url = os.getenv(
            "OSTIUM_TESTNET_RPC_URL", "https://sepolia-rollup.arbitrum.io/rpc"
        )
    else:
...
def get_sdk(
    ...
    if force_new:
        ...
        if not check_rpc_health(rpc_url, timeout=3):
            logger.warning(
                f"⚠️  Primary RPC unhealthy, trying backup: {OSTIUM_RPC_BACKUP}"
            )
            if check_rpc_health(OSTIUM_RPC_BACKUP, timeout=3):
                ...
                logger.error(
                    f"❌ Both RPCs unhealthy, but proceeding anyway (might be temporary)"
                )
    ...
    if cache_key not in sdk_cache or force_new:
        ...
        logger.info(
            f"Created SDK instance (delegation={use_delegation}, rpc={rpc_url}, network={network})"
        )

    return sdk_cache[cache_key]


def get_fresh_sdk_for_async(
    private_key: str, use_delegation: bool = False, is_testnet: bool = None
) -> OstiumSDK:
    """
    ...
    logger.debug(
        f"Created fresh SDK instance for async operations (delegation={use_delegation}, network={network})"
    )
    return sdk


def get_available_markets(refresh=False, is_testnet=None):
    """
    ...
    if (
        not refresh
        and available_markets_cache["markets"] is not None
        and available_markets_cache.get("network") == network
    ):
    ...
    try:
        ...
        response = requests.get(
            f"{api_url}/api/venue-markets/available?venue=OSTIUM", timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            if data.get("success") and data.get("markets"):
                ...
                available_markets_cache["network"] = network
...
def send_insufficient_funds_telegram_notification(
    user_address: str, market: str, collateral: float
):
    """
    ...
    try:
        ...
        logger.warning(
            f"   DATABASE_URL configured: {'✅ Yes' if database_url else '❌ No'}"
        )
        logger.warning(
            f"   TELEGRAM_BOT_TOKEN configured: {'✅ Yes' if bot_token else '❌ No'}"
        )

        if not database_url or not bot_token:
            logger.warning(
                "❌ Cannot send Telegram notification: DATABASE_URL or TELEGRAM_BOT_TOKEN not configured"
            )
            return False
        ...
        if not user_wallet_lower:
            logger.warning(
                "❌ Cannot send Telegram notification: No user address provided"
            )
            cur.close()
            conn.close()
            return False
        ...
        logger.warning(
            f"   Checking user_telegram_notifications for wallet {user_wallet_lower[:10]}..."
        )
        cur.execute(
            """
        ...
        if not telegram_info:
            logger.warning(
                f"   ❌ No user_telegram_notifications record found for {user_wallet_lower[:10]}..."
            )
            return False

        logger.warning(f"   ✅ Found user_telegram_notifications record:")
        logger.warning(
            f"      - telegram_chat_id: {telegram_info.get('telegram_chat_id')}"
        )
        logger.warning(
            f"      - telegram_username: {telegram_info.get('telegram_username')}"
        )
        ...
        if not telegram_info.get("telegram_chat_id"):
            logger.warning(
                f"   ❌ No telegram_chat_id found for {user_wallet_lower[:10]}..."
            )
            return False
...
"""
        ...
        if response.status_code == 200:
            ...
            if result.get("ok"):
                logger.warning(
                    f"✅ Successfully sent insufficient funds notification to @{telegram_info.get('telegram_username') or chat_id}"
                )
                return True
            else:
...
def validate_market(token_symbol: str, is_testnet=None):
    """
    ...
    markets = get_available_markets(is_testnet=is_testnet)
...
def resource_usage():
    ...
    if not PSUTIL_AVAILABLE:
        return jsonify(
            {"success": False, "error": "psutil not installed. Run: pip install psutil"}
        ), 503
...
def get_balance():
    ...
    try:
        ...
        try:
            ...
            return jsonify(
                {"success": False, "error": f"Invalid address format: {str(e)}"}
            ), 400
    ...
    try:
        ...
        for trade in open_trades:
            try:
                ...
                stop_loss_price = (
                    float(int(stop_loss_raw) / 1e18) if stop_loss_raw else 0.0
                )

                take_profit_raw = trade.get("takeProfitPrice", 0)
                take_profit_price = (
                    float(int(take_profit_raw) / 1e18) if take_profit_raw else 0.0
                )
...
def open_position():
    ...
    try:
        data = request.json
        logger.info(
            f"[OPEN-POSITION] Received request with keys: {list(data.keys()) if data else 'None'}"
        )
            ...
            try:
                ...
                if user_address_row and user_address_row["ostium_agent_key_encrypted"]:
                    ...
                    try:
                        ...
                        logger.info(
                            f"✅ Found and decrypted agent key for {agent_address} from user_agent_addresses"
                        )
                    ...
                    logger.info(
                        f"Found agent key for {agent_address} in wallet_pool (legacy)"
                    )
                ...
                logger.error(traceback.format_exc())
                return jsonify(
                    {"success": False, "error": f"Failed to fetch agent key: {str(e)}"}
                ), 500
        else:
        ...
        if use_delegation and not user_address:
            return jsonify(
                {"success": False, "error": "userAddress required for delegation"}
            ), 400
        ...
        logger.info(
            f"Opening {side} position: {position_size} USDC on {market} (leverage: {leverage}x, delegation: {use_delegation})"
        )
        if use_delegation:
            ...
            try:
                ...
                if database_url:
                    ...
                    if existing_position:
                        ...
                        existing_trade_id = existing_position.get(
                            "ostium_trade_id"
                        ) or existing_position.get("entry_tx_hash")
                        existing_index = existing_position.get("ostium_trade_index")

                        logger.info(
                            f"✅ IDEMPOTENCY: Position already exists for deployment {deployment_id[:8]}... + signal {signal_id[:8]}..."
                        )
                        logger.info(
                            f"   Returning existing order (tradeId: {existing_trade_id})"
                        )
                        ...
                        return jsonify(
                            {
                                ...
                                "transactionHash": existing_position.get(
                                    "entry_tx_hash", ""
                                ),
        ...
        for sdk_attempt in range(max_sdk_retries):
            try:
                ...
                if is_network_error and sdk_attempt < max_sdk_retries - 1:
                    ...
                    logger.warning(
                        f"⚠️  SDK initialization failed (attempt {sdk_attempt + 1}/{max_sdk_retries}): {error_str[:150]}"
                    )
        ...
        try:
            ...
            if price_network == "testnet":
                price_rpc_url = os.getenv(
                    "OSTIUM_TESTNET_RPC_URL", "https://sepolia-rollup.arbitrum.io/rpc"
                )
            else:
                price_rpc_url = os.getenv(
                    "OSTIUM_MAINNET_RPC_URL", "https://arb1.arbitrum.io/rpc"
                )

            price_sdk = OstiumSDK(
                network=price_network, private_key=dummy_key, rpc_url=price_rpc_url
            )
            ...
            try:
                ...
                price_result = loop.run_until_complete(
                    price_sdk.price.get_price(market.upper(), "USD")
                )
                loop.close()
                ...
                if isinstance(price_result, tuple) and len(price_result) >= 1:
                    ...
                    logger.info(
                        f"✅ Current {market} price from Ostium: ${current_price}"
                    )
                else:
                    raise Exception("Invalid price format")
            except Exception as price_error:
                logger.warning(
                    f"Could not fetch Ostium price for {market}: {price_error}"
                )
        ...
        logger.info(
            f"ℹ️  Stop-Loss will be set after position opens: {(stop_loss_percent * 100):.1f}%"
        )
        logger.info(
            f"💰 TP will NOT be set - monitoring service handles trailing stops"
        )
        ...
        for attempt in range(max_retries):
            try:
                logger.info(
                    f"🔄 Attempting trade (attempt {attempt + 1}/{max_retries})..."
                )
                ...
                if is_network_error and attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"⚠️  Network/connection error (attempt {attempt + 1}/{max_retries})"
                    )
                    logger.warning(
                    ...
                    try:
                        ...
                        if cache_key in sdk_cache:
                            del sdk_cache[cache_key]
                        sdk = get_sdk(
                            private_key, use_delegation, is_testnet=is_testnet
                        )
                    ...
                    if attempt == max_retries - 1 and is_network_error:
                        ...
                        logger.error(
                            f"   This might be a temporary RPC issue - try again later"
                        )
        ...
        try:
            ...
            try:
                open_trades = loop.run_until_complete(
                    async_sdk.subgraph.get_open_trades(trader_address)
                )
            finally:
                loop.close()
            ...
            if len(open_trades) > 0:
                ...
                for trade in open_trades:
                    ...
                    trade_collateral = (
                        float(int(trade_collateral_raw)) / 1e6
                        if trade_collateral_raw
                        else 0
                    )

                    if (
                        trade_market == market.upper()
                        and trade_side == side.lower()
                        and abs(trade_collateral - position_size) / position_size < 0.05
                    ):
                ...
                if newly_opened_trade:
                    ...
                    trade_entry_price = (
                        float(int(newly_opened_trade.get("openPrice", 0)) / 1e18)
                        if newly_opened_trade.get("openPrice")
                        else 0
                    )
                    ...
                    if trade_pair_id != str(asset_index):
                        logger.warning(
                            f"⚠️  Pair mismatch! Expected {asset_index}, got {trade_pair_id}"
                        )
                        logger.warning(
                            f"   This might be a subgraph delay - position monitor will verify"
                        )
                else:
                    logger.warning(
                        f"⚠️  No matching open trade found yet - order may not be filled"
                    )
                    logger.warning(
                        f"   Position monitor will update index once position is discovered"
                    )
                    actual_trade_index = None
            else:
                logger.warning(f"⚠️  No open trades found yet - order may not be filled")
                logger.warning(
                    f"   Position monitor will set TP/SL once trade is filled"
                )
                actual_trade_index = None
            ...
            logger.warning(
                f"   Position monitor will update index once order is filled"
            )
        ...
        if actual_trade_index is not None:
            ...
            logger.info(
                f"ℹ️  Index not available yet (order pending or delegation issue)"
            )
            logger.info(
                f"   Position monitor will update index once position is discovered"
            )
        ...
        sl_error = None
        logger.info(
            f"ℹ️  SL will be set by position monitor once keeper fills the order"
        )
        logger.info(
            f"   Reason: Need to wait for keeper (1-5 min) and use user's filled position for accurate liquidation price"
        )
        ...
        if "eca695e1" in error_str.lower() or "belowminlevpos" in error_str.lower():
            logger.warning(
                "⚠️  BelowMinLevPos error detected - user has insufficient funds"
            )
            if user_address and market and position_size:
                try:
                    logger.warning(
                        f"Attempting to notify user {user_address[:10]}... via Telegram"
                    )
                    send_insufficient_funds_telegram_notification(
                        user_address, market, position_size
                    )
...
def close_position():
    ...
    try:
        ...
        async_sdk = get_fresh_sdk_for_async(
            private_key, use_delegation, is_testnet=is_testnet
        )
        ...
        try:
            result = loop.run_until_complete(
                async_sdk.get_open_trades(trader_address=address_to_check)
            )
        finally:
            loop.close()
        ...
        if not trade_to_close:
            ...
            return jsonify(
                {"success": True, "message": "No open position to close", "closePnl": 0}
            )
        ...
        if not stored_trade_index and trade_id:
            try:
                ...
                if db_position and db_position.get("ostium_trade_index") is not None:
                    ...
                    logger.info(
                        f"📦 Found stored trade index in DB: {stored_trade_index}"
                    )
        ...
        if stored_trade_index is not None:
            ...
            logger.info(
                f"✅ Using STORED trade index: {trade_index} (from database/request)"
            )
        else:
            trade_index = 0
            logger.warning(
                f"⚠️  No stored index found, using index=0 (may close wrong position!)"
            )
            logger.warning(
                f"   This works ONLY if there's ONE position per market per user"
            )
        ...
        try:
            ...
            if market_data:
                ...
                logger.info(
                    f"Found pair_index for {market} from venue_markets table: {pair_index}"
                )
            else:
                ...
                if isinstance(pair_info, dict):
                    ...
                    logger.warning(
                        f"Market {market} not in venue_markets, using trade data: {pair_index}"
                    )
                else:
                    pair_index = None
        ...
        logger.info(
            f"🎯 Closing tradeID {target_trade_id} for {market} using index={trade_index}"
        )
        ...
        logger.info(
            f"Closing position: {market} (trade_index: {trade_index}, pair_index: {pair_index})"
        )
        ...
        print(
            f"[CLOSE] Calling close_trade: trade_index={trade_index}, pair_id={pair_index}, price={current_price}"
        )
        logger.info(
            f"Calling close_trade: trade_index={trade_index}, pair_id={pair_index}, price={current_price}"
        )
        ...
        try:
            if use_delegation:
                ...
                try:
                    ...
                    logger.error(
                        f"Invalid user_address format: {user_address}, error: {e}"
                    )
                    raise ValueError(f"Invalid userAddress format: {user_address}")

                print(
                    f"[CLOSE] Using delegation - closing on behalf of {checksummed_user_address}"
                )
                logger.info(
                    f"Using delegation - closing on behalf of {checksummed_user_address}"
                )
            ...
            if isinstance(result, tuple) and len(result) >= 2:
                ...
                if error_hex.startswith("0xf77a8069"):
                    ...
                    logger.error(
                        f"❌ Position already closed or doesn't exist (error: {error_hex})"
                    )
            ...
            if not result:
                print(
                    f"[CLOSE] ❌ SDK returned empty result! Position might not be closeable yet."
                )
                logger.error(
                    f"❌ SDK returned empty result! Position might not be closeable yet."
                )
                return jsonify(
                    {
            ...
            if isinstance(sdk_error, Web3RPCError):
                if (
                    "insufficient funds" in error_str.lower()
                    or "insufficient funds" in error_message.lower()
                ):
            ...
            if is_insufficient_funds:
                logger.error(
                    f"❌ Insufficient ETH for gas on agent address: {agent_address}"
                )
            ...
            if is_position_closed_error:
                # This is "NoOpenPosition" or "PositionAlreadyClosed" error
                logger.info(
                    f"✅ Position already closed or doesn't exist (error code: 0xf77a8069)"
                )
                logger.info(
                    f"   This is normal if position was closed externally or doesn't exist"
                )
                return jsonify(
                    {
                        "success": True,  # Treat as success (idempotent)
...
def transfer_usdc():
    ...
    try:
        ...
        result = sdk.ostium.withdraw(amount=amount, receiving_address=to_address)
...
def get_trade_by_id():
    ...
    try:
        ...
        return jsonify({"success": True, "tradeId": trade_id, "trade": trade})
    except Exception as e:
...
def get_closed_trade_by_id():
    ...
    try:
        ...
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        closed_orders = loop.run_until_complete(
            sdk.subgraph.get_closed_trade_by_trade_id(trade_id)
        )
        loop.close()

        logger.info(
            f"Found {len(closed_orders)} closed orders for trade ID: {trade_id}"
        )
...
def get_cancelled_orders_by_id():
    ...
    try:
        ...
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        cancelled_orders = loop.run_until_complete(
            sdk.subgraph.get_cancelled_orders_by_trade_id(trade_id)
        )
        loop.close()

        logger.info(
            f"Found {len(cancelled_orders)} cancelled orders for trade ID: {trade_id}"
        )
...
def approve_agent():
    ...
    try:
        ...
        sdk = OstiumSDK(network=network, private_key=user_key, rpc_url=rpc_url)
        ...
        tx = trading_contract.functions.setDelegate(
            checksummed_agent_address
        ).build_transaction(
            {
...
def request_faucet():
    ...
    try:
        ...
        if not OSTIUM_TESTNET:
            return jsonify(
                {"success": False, "error": "Faucet only available on testnet"}
            ), 400
...
def get_market_info():
    ...
    try:
        network_name, rpc_url = get_network_config(request)
        network = (
            NetworkConfig.testnet()
            if network_name == "testnet"
            else NetworkConfig.mainnet()
        )
        sdk = OstiumSDK(network=network, rpc_url=rpc_url)
...
def available_markets():
    ...
    try:
        ...
        network, _ = get_network_config(request)
...
def validate_market_endpoint():
    ...
    try:
        ...
        asset_index, is_available, market_name = validate_market(
            market,
            is_testnet=(network == "testnet"),
        )
...
def get_closed_positions():
    ...
    try:
        ...
        for item in history:
            ...
            profit_percent = (
                float(int(profit_percent_raw)) / 1e6 if profit_percent_raw else 0
            )

            total_profit_percent_raw = item.get("totalProfitPercent", 0)
            total_profit_percent = (
                float(int(total_profit_percent_raw)) / 1e6
                if total_profit_percent_raw
                else 0
            )

            pnl_usdc = (
                amount_sent - collateral_usdc if order_action in close_actions else 0
            )
...
def set_stop_loss():
    ...
    try:
        ...
        logger.info(
            f"Setting SL for {market} (trade_index={trade_index}, pair_index={pair_index})"
        )
        logger.info(f"   User: {user_address}, Agent: {agent_address}")
        logger.info(
            f"   SL%: {stop_loss_percent * 100:.1f}%, Entry Price: ${entry_price:.4f}"
        )
        ...
        try:
            ...
            if (
                not user_address_row
                or not user_address_row["ostium_agent_key_encrypted"]
            ):
                cur.close()
                conn.close()
                return jsonify(
                    {
                        "success": False,
                        "error": f"Agent address {agent_address} not found",
                    }
                ), 404
        ...
        try:
            ...
            metrics = loop.run_until_complete(
                sdk.get_open_trade_metrics(
                    pair_index, trade_index, trader_address=checksummed_user
                )
            )
            loop.close()
            ...
            if liquidation_price:
                ...
                logger.warning(
                    f"   ⚠️  Liquidation price not available from SDK (metrics keys: {list(metrics.keys()) if metrics else 'None'})"
                )
        ...
        logger.info(
            f"   Initial SL: ${sl_price:.4f} ({stop_loss_percent * 100:.1f}% from entry)"
        )
        ...
        if liquidation_price and liquidation_price > 0:
            ...
            if is_long:
                ...
                if sl_price < min_safe_sl:
                    logger.warning(
                        f"   ⚠️  SL ${sl_price:.4f} too close to liq ${liquidation_price:.4f}"
                    )
                    sl_price = min_safe_sl
                    adjusted_percent = ((entry_price - sl_price) / entry_price) * 100
                    logger.info(
                        f"   ✅ Adjusted SL: ${sl_price:.4f} ({adjusted_percent:.1f}% below entry, 2% above liq)"
                    )
            else:
                ...
                if sl_price > max_safe_sl:
                    ...
                    logger.info(
                        f"   ✅ Adjusted SL: ${sl_price:.4f} ({adjusted_percent:.1f}% above entry, 2% below liq)"
                    )
        else:
            SAFETY_BUFFER = 0.02
            ...
            if stop_loss_percent > MAX_SAFE_SL_PERCENT:
                logger.warning(
                    f"   ⚠️  Requested SL {stop_loss_percent * 100:.1f}% may be at/beyond liquidation (liq price unavailable)"
                )
                logger.warning(
                    f"   ⚠️  Capping SL to {MAX_SAFE_SL_PERCENT * 100:.1f}% for safety"
                )
                ...
                logger.info(
                    f"   ✅ Adjusted SL: ${sl_price:.4f} ({stop_loss_percent * 100:.1f}% from entry)"
                )
        ...
        try:
            if use_delegation:
                ...
                sdk.ostium.update_sl(
                    pair_index, trade_index, sl_price, checksummed_user
                )
            else:
            ...
            logger.error(
                f"   Details: pair_index={pair_index}, trade_index={trade_index}, sl_price=${sl_price:.4f}"
            )
            logger.error(
                f"   Entry: ${entry_price:.4f}, SL%: {stop_loss_percent * 100:.2f}%, Side: {'LONG' if is_long else 'SHORT'}"
            )
...
def set_take_profit():
    ...
    try:
        ...
        entry_price = float(
            data.get("entryPrice")
        )  # Use entry price, not current price
        pair_index = data.get("pairIndex")
        use_delegation = data.get("useDelegation", True)
        ...
        logger.info(
            f"Setting TP for {market} (trade_index={trade_index}, pair_index={pair_index})"
        )
        logger.info(f"   User: {user_address}, Agent: {agent_address}")
        logger.info(
            f"   TP%: {take_profit_percent * 100:.1f}%, Entry Price: ${entry_price:.4f}"
        )
        ...
        tp_price = (
            ...
            if is_long
            else entry_price * (1 - take_profit_percent)
        )

        logger.info(
            f"   Entry: ${entry_price:.4f} → TP: ${tp_price:.4f} ({take_profit_percent * 100:.1f}% {'above' if is_long else 'below'} entry)"
        )
        ...
        try:
            if use_delegation:
                ...
                sdk.ostium.update_tp(
                    pair_index, trade_index, tp_price, checksummed_user
                )
            else:
            ...
            return jsonify({"success": False, "error": str(tp_error)}), 500
...
def get_price(token):
    ...
    try:
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            price_result = loop.run_until_complete(
                sdk.price.get_price(token.upper(), "USD")
            )
            loop.close()

            logger.info(
                f"Raw price result for {token}: {price_result} (type: {type(price_result)})"
            )
        except Exception as sdk_error:
...
if __name__ == "__main__":
    ...
    logger.info(
        f"   Network: {'TESTNET (Arbitrum Sepolia)' if OSTIUM_TESTNET else 'MAINNET'}"
    )

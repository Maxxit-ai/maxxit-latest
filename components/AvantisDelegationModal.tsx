/**
 * Avantis Delegation Modal - For setting up delegation to agent address on Base chain
 * Uses Avantis SDK's setDelegate function via the trading contract
 */
...
interface AvantisDelegationModalProps {
    agentAddress: string;
    onClose: () => void;
    onSuccess?: () => void;
}

// Get Avantis configuration based on environment
const { tradingContract: AVANTIS_TRADING_CONTRACT, chainId: BASE_CHAIN_ID, blockExplorerUrl, chainName } = getAvantisConfig();
const AVANTIS_TRADING_ABI = ['function setDelegate(address delegate) external'];

export function AvantisDelegationModal({
    agentAddress,
    onClose,
    onSuccess,
}: AvantisDelegationModalProps) {
    ...
    const approveDelegation = async () => {
        ...
        try {
            ...
            console.log(`[AvantisDelegation] Gas estimate: ${gasEstimate.toString()}, with 50% buffer: ${gasLimit.toString()}`);
            ...
            console.log('[AvantisDelegation] ✅ Delegation approved');
            setStep('complete');
            setTxHash(null);
            ...
            console.error('[AvantisDelegation] Delegation error:', err);
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-[var(--text-muted)]">Avantis Agent (Base)</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                    ...
                    {step === 'connect' ? (
                            ...
                            <div className="text-center space-y-6 py-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-[var(--text-secondary)]">
                                        Connect your Base wallet to delegate to your agent
                                    </p>
                                </div>
                                <button
                        ...
                        <>
                            ...
                            <div className="border border-[var(--border)] p-4 space-y-3 text-sm">
                                ...
                                <div className="flex items-start gap-2 text-[var(--text-secondary)]">
                                    ...
                                    <span>Sign transaction to delegate trading permissions on Avantis (Base)</span>
                                </div>
                            </div>
                        ...
                        <div className="text-center space-y-6 py-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-[var(--text-secondary)]">
                                    Your agent is now delegated to trade on Avantis on your behalf
                                </p>
                            </div>

...
export function MultiVenueSelector({
    ...
    return (
      <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-2 sm:p-3 md:p-4">
        <div className="bg-[var(--bg-deep)] border border-[var(--border)] max-w-md w-full">
          <div className="p-4 sm:p-6 md:p-8 text-center space-y-4 sm:space-y-5 md:space-y-6">
            <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 mx-auto border border-[var(--accent)] flex items-center justify-center">
              <Zap className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 text-[var(--accent)]" />
            </div>
            <div>
              <h3 className="font-display text-lg sm:text-xl mb-1 sm:mb-2">CONNECT WALLET</h3>
              <p className="text-[var(--text-secondary)] text-xs sm:text-sm px-2">
            ...
            <div className="space-y-2 sm:space-y-3">
              <button
                onClick={() => login()}
                className="w-full py-2.5 sm:py-3 bg-[var(--accent)] text-[var(--bg-deep)] font-bold hover:bg-[var(--accent-dim)] transition-colors text-xs sm:text-sm"
              >
                CONNECT WALLET
              </button>
              <button
                onClick={onClose}
                className="w-full py-2.5 sm:py-3 border border-[var(--border)] text-[var(--text-secondary)] font-bold hover:border-[var(--text-primary)] transition-colors text-xs sm:text-sm"
              >
                CANCEL
              </button>
            </div>
  ...
  if (loading || creatingDeployments) {
    return (
      <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-2 sm:p-3 md:p-4">
        <div className="bg-[var(--bg-deep)] border border-[var(--border)] max-w-md w-full p-4 sm:p-6 md:p-8">
          <div className="text-center space-y-3 sm:space-y-4">
            {creatingDeployments ? (
              <>
                <CheckCircle className="h-12 w-12 sm:h-14 sm:w-14 md:h-16 md:w-16 mx-auto text-[var(--accent)]" />
                <h3 className="font-display text-lg sm:text-xl">JOINED!</h3>
                <p className="text-[var(--text-secondary)] text-xs sm:text-sm px-2">
              ...
              <>
                <Activity className="h-10 w-10 sm:h-12 sm:w-12 mx-auto text-[var(--accent)] animate-pulse" />
                <p className="text-[var(--text-muted)] text-xs sm:text-sm">Checking setup...</p>
              </>
            )}
          </div>
        </div>
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-1 sm:p-2 md:p-4">
        <div className="bg-[var(--bg-deep)] border border-[var(--border)] max-w-4xl w-full max-h-[98vh] sm:max-h-[95vh] md:max-h-[90vh] flex flex-col">
          {/* Header */}
          <div className="border-b border-[var(--border)] p-2 sm:p-3 md:p-4 flex items-center justify-between flex-shrink-0">
            <div className="flex-1 min-w-0 pr-2">
              <h2 className="font-display text-base sm:text-lg md:text-xl truncate">{agentName}</h2>
              {agentDescription && <p className="text-[10px] sm:text-xs text-[var(--text-secondary)] pb-1 sm:pb-2 line-clamp-2">{agentDescription}</p>}
              <p className="text-[10px] sm:text-xs text-[var(--text-muted)] mt-0.5 sm:mt-1">Select a venue to join</p>
            </div>
            <button
              onClick={onClose}
              className="p-1 sm:p-1.5 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors flex-shrink-0"
            >
          ...
          >
            {/* Notification Toast */}
            {notification && (
              <div className="mb-3 sm:mb-4 p-2 sm:p-3 border border-[var(--accent)] bg-[var(--accent)]/10 text-[var(--accent)] text-xs sm:text-sm flex items-center gap-2">
              ...
              {venues.map((venue) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3 sm:p-4 border transition-all text-center ${venue.disabled ? 'disabled:opacity-50 disabled:cursor-not-allowed' : ''} ${isAlreadySetup
                      ? 'border-[var(--accent)] bg-[var(--accent)]/5 cursor-pointer'
                      : 'border-[var(--border)] hover:border-[var(--accent)]'
                      }`}
                  >
                    <div className="space-y-1.5 sm:space-y-2">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 border border-[var(--accent)] flex items-center justify-center mx-auto">
                        <Zap className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[var(--accent)]" />
                      </div>
                      <h3 className="font-display text-xs sm:text-sm">{venue.name}</h3>
                      <p className="text-[10px] sm:text-xs text-[var(--text-muted)]">{venue.description}</p>

                      {!venue.disabled && !isAlreadySetup && (
                        <div className="space-y-1 text-[10px] sm:text-xs text-[var(--text-muted)] mt-2 sm:mt-3">
                      ...
                      {venue.disabled ? (
                        <div className="mt-2 px-2 sm:px-3 py-0.5 sm:py-1 border border-[var(--border)] text-[var(--text-muted)] text-[10px] sm:text-xs inline-block">
                          COMING SOON
                        </div>
                      ) : isAlreadySetup ? (
                        <div className="mt-2 space-y-0.5 sm:space-y-1">
                          <div className="px-2 sm:px-3 py-0.5 sm:py-1 bg-[var(--accent)] text-[var(--bg-deep)] font-bold text-[10px] sm:text-xs inline-flex items-center gap-1 sm:gap-1.5">
                            <CheckCircle className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                            ACTIVE
                          </div>
                          <p className="text-[10px] sm:text-xs text-[var(--text-muted)] italic">Click for details</p>
                        </div>
                      ) : (
                        <div className="mt-2 px-2 sm:px-3 py-0.5 sm:py-1 border border-[var(--accent)] text-[var(--accent)] font-bold text-[10px] sm:text-xs inline-flex items-center gap-1 sm:gap-1.5">
                          SETUP
            ...
            {userAgentAddresses && (userAgentAddresses.hyperliquid || userAgentAddresses.ostium) && (
              <div className="border border-[var(--accent)]/40 bg-[var(--accent)]/5 p-3 sm:p-4 mt-4 sm:mt-6">
                <p className="data-label mb-2 sm:mb-3 text-[10px] sm:text-xs">YOUR AGENT WALLETS</p>
                <div className="space-y-2 sm:space-y-3">
                  ...
                  {userAgentAddresses.ostium && (
                    <div className="flex items-center justify-between gap-2 sm:gap-3 p-2 sm:p-3 bg-[var(--bg-deep)] border border-[var(--border)] hover:border-[var(--accent)]/50 transition-colors">
                      <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                        <div className="w-7 h-7 sm:w-8 sm:h-8 border-2 border-[var(--accent)]/60 flex items-center justify-center flex-shrink-0">
                          <Wallet className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[var(--accent)]" />
                        </div>
                        <div className="min-w-0">
                      ...
                      >
                        {copiedAddress === `ostium-${userAgentAddresses.ostium}` ? (
                          <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[var(--accent)]" />
                        ) : (
            ...
            <div className="border border-[var(--border)] p-3 sm:p-4 mt-4 sm:mt-6">
              <p className="data-label mb-3 sm:mb-4 text-[10px] sm:text-xs">SETUP INSTRUCTIONS</p>
              <div className="space-y-3 sm:space-y-4">
                {/* Step 1 */}
                <div className="flex gap-2 sm:gap-3 md:gap-4">
                  <div className="flex-shrink-0">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 border-2 border-[var(--accent)] flex items-center justify-center text-[var(--accent)] font-bold text-xs sm:text-sm">
                      1
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-bold text-[var(--text-primary)] mb-0.5 sm:mb-1">Select a Venue</p>
                ...
                <div className="flex gap-2 sm:gap-3 md:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-bold text-[var(--text-primary)] mb-0.5 sm:mb-1">Configure Trading Preferences</p>
                    <p className="text-[10px] sm:text-xs text-[var(--text-secondary)]">
                ...
                <div className="flex gap-2 sm:gap-3 md:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-bold text-[var(--text-primary)] mb-0.5 sm:mb-1">
                ...
                <div className="flex gap-2 sm:gap-3 md:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-bold text-[var(--text-primary)] mb-0.5 sm:mb-1">Approve USDC</p>
                    <p className="text-[10px] sm:text-xs text-[var(--text-secondary)]">
                ...
                <div className="flex gap-2 sm:gap-3 md:gap-4">
                  <div className="flex-shrink-0">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 border-2 border-[var(--accent)] bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)] font-bold text-xs sm:text-sm">
                      ✓
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-bold text-[var(--text-primary)] mb-0.5 sm:mb-1">Agent Active</p>
                    <p className="text-[10px] sm:text-xs text-[var(--text-secondary)]">
              ...
              <div className="border border-[var(--accent)]/40 bg-[var(--accent)]/5 p-3 sm:p-4 mt-4 sm:mt-6">
                ...
                <div className="mb-3 sm:mb-4 p-2.5 sm:p-3 bg-[var(--bg-deep)] border border-[var(--border)]">
                  <p className="text-[10px] sm:text-xs font-bold text-[var(--text-primary)] mb-1.5 sm:mb-2 flex items-center gap-1.5 sm:gap-2">
                    <Send className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[var(--accent)] flex-shrink-0" />
                    Fund Agent with ETH
                  </p>
                  <p className="text-[10px] sm:text-xs text-[var(--text-secondary)] mb-2 sm:mb-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      className="flex-1 px-2 sm:px-3 py-1.5 sm:py-2 bg-[var(--bg-surface)] border border-[var(--border)] text-[var(--text-primary)] text-xs sm:text-sm focus:outline-none focus:border-[var(--accent)]"
                    />
                    <button
                      onClick={handleSendETH}
                      disabled={sendingEth || !ethAmount || parseFloat(ethAmount) <= 0}
                      className="w-full sm:w-auto px-3 sm:px-4 py-1.5 sm:py-2 bg-[var(--accent)] text-[var(--bg-deep)] font-bold text-[10px] sm:text-xs hover:bg-[var(--accent-dim)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1.5 sm:gap-2"
                    >
                      {sendingEth ? (
                        <>
                  ...
                  {ethTxHash && (
                    <p className="text-[10px] sm:text-xs text-[var(--accent)] mt-1.5 sm:mt-2 font-mono break-all">
                      TX: {ethTxHash.slice(0, 20)}...
                    </p>
                  )}
                  {ethError && (
                    <p className="text-[10px] sm:text-xs text-red-500 mt-1.5 sm:mt-2">
                      {ethError}
                    </p>
                  )}
                </div>
                ...
                <div className="p-2.5 sm:p-3 bg-[var(--bg-deep)] border border-[var(--border)]">
                  <p className="text-[10px] sm:text-xs font-bold text-[var(--text-primary)] mb-1.5 sm:mb-2 flex items-center gap-1.5 sm:gap-2">
                    <ExternalLink className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[var(--accent)] flex-shrink-0" />
                    Register on Ostium Platform
                  </p>
                  <p className="text-[10px] sm:text-xs text-[var(--text-secondary)] mb-2 sm:mb-3">
                    To enable trading, you need to register on the Ostium platform and deposit USDC:
                  </p>
                  <ol className="space-y-1.5 sm:space-y-2 text-[10px] sm:text-xs text-[var(--text-secondary)] ml-3 sm:ml-4 list-decimal">
                    <li>
                      Visit{' '}
                      <a
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-2 sm:mt-3 inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-[var(--accent)] text-[var(--bg-deep)] font-bold text-[10px] sm:text-xs hover:bg-[var(--accent-dim)] transition-colors w-full sm:w-auto justify-center"
                  >
            ...
            {(setupStatus?.hasHyperliquid || setupStatus?.hasOstium) && (
              <button
                onClick={onComplete}
                className="w-full mt-3 sm:mt-4 py-2.5 sm:py-3 bg-[var(--accent)] text-[var(--bg-deep)] font-bold hover:bg-[var(--accent-dim)] transition-colors mb-3 sm:mb-4 text-xs sm:text-sm"
              >
                DONE
              </button>
            )}

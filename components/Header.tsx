...
export function Header() {
  ...
  const [isCrossAppLinking, setIsCrossAppLinking] = useState(false);
  const [crossAppActionError, setCrossAppActionError] = useState<string | null>(null);
  ...
  const hasCrossAppProviderConfigured = Boolean(crossAppProviderId);
  const walletSourceLabel = walletSource === 'cross_app'
    ? 'Global wallet'
    : walletSource === 'embedded'
      ? 'Embedded wallet'
      : walletSource === 'connected_wallet'
        ? 'Connected wallet'
  ...
  useEffect(() => {
    if (authenticated && isOnArbitrum && resolvedWalletAddress) {
  ...
  }, [authenticated, isOnArbitrum, resolvedWalletAddress]);
  ...
  }, [authenticated, resolvedWalletAddress]);
  ...
  const handleSwitchToArbitrum = async () => {
    const syncCurrentChain = async (provider: any) => {
      const chainId = await provider.request({ method: 'eth_chainId' });
      setCurrentChainId(parseInt(chainId as string, 16));
    };

    const switchToActiveNetwork = async (provider: any) => {
    ...
    try {
      ...
      await syncCurrentChain(provider);
      ...
      if (switchError.code === 4902) {
        try {
          ...
          // Some wallets add the network but do not auto-select it.
          await switchToActiveNetwork(provider);
          await syncCurrentChain(provider);
        } catch (addError) {
          console.error('Failed to add network:', addError);
        }
      } else if (switchError.code === -32601 || switchError.code === 4200) {
        // Fallback for providers that don't expose wallet_switchEthereumChain.
        try {
          const activeWallet = wallets.find((wallet) => wallet.address?.toLowerCase() === resolvedWalletAddress?.toLowerCase());
          const switchChain = (activeWallet as any)?.switchChain;
          if (typeof switchChain === 'function') {
            await switchChain(ACTIVE_NETWORK.chainId);
            const provider = await getEip1193Provider();
            await syncCurrentChain(provider);
            return;
          }
        } catch (fallbackError) {
          console.error('Fallback switch chain failed:', fallbackError);
        }
      }
  ...
  const handleTransferSubmit = async () => {
    ...
    try {
      ...
      setTransferAmount('');
      fetchUsdcBalance(resolvedWalletAddress, false);
    } catch (error: any) {
  ...
  const handleCrossAppLink = async () => {
    if (!crossAppProviderId) return;
    setCrossAppActionError(null);
    setIsCrossAppLinking(true);
    try {
      await linkCrossAppAccount({ appId: crossAppProviderId });
    } catch (error: any) {
      setCrossAppActionError(error?.message || 'Failed to link global wallet account.');
    } finally {
      setIsCrossAppLinking(false);
    }
  };
  ...
  const handleCrossAppLogin = async () => {
    ...
    setIsCrossAppLinking(true);
    try {
      await loginWithCrossAppAccount({ appId: crossAppProviderId });
    } catch (error: any) {
      setCrossAppActionError(error?.message || 'Failed to log in with global wallet.');
    } finally {
      setIsCrossAppLinking(false);
    }
  };
              ...
              {ready && (
                <div className="relative ml-2">
                  {authenticated ? (
                      ...
                      {isWalletModalOpen && !needsNetworkSwitch && (
                        ...
                        >
                          <div className="p-4 space-y-4">
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex-1">
                                  <p className="font-mono text-xs break-all text-[var(--text-primary)] leading-relaxed mb-2">
                                    {resolvedWalletAddress || 'No address'}
                                  </p>
                                  <p className="text-[10px] uppercase tracking-wide text-[var(--accent)] mb-2">
                                    {walletSourceLabel}
                                  </p>
                                  {walletSource === 'cross_app' && providerAppIdMatched && (
                                    <p className="text-[10px] text-[var(--text-secondary)] mb-2">
                                      Linked via provider app
                                    </p>
                                  )}
                            ...
                            {hasCrossAppProviderConfigured && authenticated && !providerAppIdMatched && (
                              <div className="p-3 bg-[var(--bg-elevated)] border border-[var(--border)] space-y-2">
                                <p className="text-xs text-[var(--text-secondary)]">
                                  Link your global wallet account to reuse the same wallet across apps.
                                </p>
                                <button
                                  onClick={handleCrossAppLink}
                                  disabled={isCrossAppLinking}
                                ...
                                >
                                  {isCrossAppLinking ? 'Linking...' : 'Link Global Wallet'}
                                </button>
                              </div>
                            )}
                            {crossAppActionError && (
                              <p className="text-xs text-[var(--danger)]">{crossAppActionError}</p>
                            )}
                    ...
                    <div className="flex items-center gap-2">
                      {hasCrossAppProviderConfigured && (
                        <button
                          onClick={handleCrossAppLogin}
                          disabled={isCrossAppLinking}
                          className="inline-flex items-center justify-center gap-2 px-4 py-2 border border-[var(--accent)] text-[var(--accent)] text-sm font-bold hover:bg-[var(--accent)]/10 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <Wallet className="h-4 w-4" />
                          <span className="hidden sm:inline">{isCrossAppLinking ? 'CONNECTING...' : 'GLOBAL WALLET'}</span>
                        </button>
                      )}
                      <button
                        onClick={login}

  ...
  return (
    <div 
      className={`bg-[var(--bg-surface)] border rounded-xl overflow-hidden transition-all duration-300 ${
        isExpanded ? 'border-[var(--accent)] shadow-[0_0_30px_var(--accent-glow)]' : 'border-[var(--border)] hover:border-[var(--accent)]/50'
      }`}
    >
      {/* Header */}
      <button
      ...
      >
        <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold transition-colors ${
          isExpanded ? 'bg-[var(--accent)] text-[var(--bg-deep)]' : 'bg-[var(--bg-elevated)] text-[var(--text-primary)]'
        }`}>
          {step.number}
        </div>
        <div className="flex-grow">
          <div className="flex items-center gap-3 mb-1">
            <Icon className={`h-5 w-5 ${isExpanded ? 'text-[var(--accent)]' : 'text-[var(--text-secondary)]'}`} />
      ...
      {isExpanded && (
        <div className="px-6 pb-6 space-y-6">
          ...
          <div className="space-y-3">
            <h4 className="font-semibold text-[var(--text-primary)]">Steps:</h4>
            <ol className="space-y-2">
              {step.details.map((detail, index) => (
                <li key={index} className="flex items-start gap-3">
                  <span className="flex-shrink-0 w-6 h-6 rounded-full bg-[var(--accent)]/20 text-[var(--accent)] text-sm font-semibold flex items-center justify-center">
                    {index + 1}
                  </span>
                  <span className="text-[var(--text-primary)] text-sm">{detail}</span>
                </li>
              ))}
            </ol>
          </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[var(--accent)] to-[var(--accent)]/60 flex items-center justify-center shadow-[0_0_20px_var(--accent-glow)]">
              <svg className="w-7 h-7 text-[var(--bg-deep)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* Quick intro */}
          <div className="p-6 rounded-xl bg-gradient-to-r from-[var(--accent)]/10 to-transparent border border-[var(--accent)]/30 mt-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="lg:col-span-1">
            <div className="sticky top-28 bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl p-4 max-h-[calc(100vh-8rem)] overflow-y-auto">
              {/* Join Club Section */}
              ...
              <nav className="space-y-1 mb-4">
                {joinClubSteps.map((step) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`flex-shrink-0 w-5 h-5 rounded-full text-[10px] font-semibold flex items-center justify-center ${
                        isActive 
          ...
          <main className="lg:col-span-3 space-y-4">
            ...
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-xl font-bold text-[var(--text-primary)]">Join an Alpha Club</h2>
                  <p className="text-sm text-[var(--text-secondary)]">Follow these steps to join an existing club and start trading</p>
                </div>
              </div>
            ...
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-xl font-bold text-[var(--text-primary)]">Create Your Own Club</h2>
                  <p className="text-sm text-[var(--text-secondary)]">Want to build your own strategy? Here's how to create a club</p>
                </div>
              </div>

              <div className="p-4 rounded-lg bg-[var(--accent)]/10 border border-[var(--accent)]/30 mb-6">
            ...
            {/* Completion Message */}
            <div className="mt-8 p-8 rounded-xl bg-gradient-to-r from-[var(--accent)]/20 to-[var(--accent)]/5 border border-[var(--accent)]/30 text-center">
              <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-[var(--accent)] flex items-center justify-center">
                <CheckCircle2 className="h-8 w-8 text-[var(--bg-deep)]" />
              </div>
              <h3 className="text-2xl font-bold text-[var(--text-primary)] mb-2">You're All Set!</h3>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link href="/">
                  <button className="px-6 py-3 bg-[var(--accent)] text-[var(--bg-deep)] font-semibold rounded-lg hover:opacity-90 transition-opacity">
                    Browse Alpha Clubs
                  </button>
                </Link>
                <Link href="/create-agent">
                  <button className="px-6 py-3 border border-[var(--accent)] text-[var(--accent)] font-semibold rounded-lg hover:bg-[var(--accent)]/10 transition-colors">
            ...
            {/* Help Section */}
            <div className="mt-6 p-6 rounded-xl bg-[var(--bg-surface)] border border-[var(--border)]">
              <h3 className="font-semibold text-[var(--text-primary)] mb-2">Need More Help?</h3>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <a href="https://t.me/maxxit" target="_blank" rel="noopener noreferrer" className="text-sm text-[var(--accent)] hover:underline">
